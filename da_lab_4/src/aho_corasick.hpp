...
struct TResults{
    ...
    TUll Word; 
    TUll Sample;
    TResults(TUll line, TUll word, TUll sample): 
            Line(line), Word(word), Sample(sample){}
};
...
struct TNode{
    ...
    TNode(std::string& val, TNode* p):
                    Value(val), Line(0), Length(0), Par(p), SuffLink(nullptr), TerminalLink(nullptr){} 
